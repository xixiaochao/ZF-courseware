...
folders.addEventListener('mousedown',function(ev){
   ...
   // let id = breadNav.getElenmentsByTayName('span')[0].dataset.id*1;
    ...
    for(let i=0;i<fileItem.length;i++){
        ...
        //
        if(arr && arr[i]){
            arr[i].checked = false;//在down的时候吧数据checked清除
    ...
    let move = function(ev){
        ...
        2.打钩
        3.吧碰到的元素对应的数据id存起来
        4.把碰到的元素对应额数据中的checked设置为true

        如果被碰到
            1.删除文件夹的active
            2.取消打钩
            3.把没碰到的元素对应的数据中的checked设置为false
*/
...
//循环子级数据,把子级数据的checked编程和checkadd的class一致的布尔值
        arr && arr.forEach(e=>{
