...
let iconclose = modaltree.getElementsByClassName('icon_close')[0];
let btn1 = footer.querySelector('input[value="确定"]');  
let btn2 = footer.querySelector('input[value="取消"]'); 
...
remove.onclick = function(){
    if(seleEleArr.length == 0){
        ...
        //关闭和取消按钮点击，弹框消失
        iconclose.onclick = btn2.onclick = function(){
...
btn1.onclick = function(){
    ...
    if(arr == null)arr = []
        ...
        for(let i=0;i<seleEleArr.length;i++){
            ...
            if(arr1 == null)arr1 = []
                let judge = seleEleArr[i].id !== sid && !arr1.includes(data[sid])
                //没有移动到他自身，或者他的子集上，则为true，否则为false
                num = num*judge;
            }
            ...
            if(num ==1){//符合条件，每一项的pid 都应该等于 被选择性的id(sid)
                for(let i=0;i<seleEleArr.length;i++){
                    ...
                    //如果子元素中有重复的名字则为false，没有重复项则为true。
                    if(result){//如果没有重复项
